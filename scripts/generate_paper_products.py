...
def add_derived(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["dmag_obs_minus_pred"] = out["Mag_Kron"] - out["mag"]
    return out
...
def make_tables(df: pd.DataFrame, outdir: Path) -> dict[str, pd.DataFrame]:
    overall = pd.DataFrame(
        [
            ("Matched detections", f"{len(df):,}", "Rows in final matched table"),
            ("Unique known objects", f"{df['query_id'].nunique():,}", "Unique query_id"),
            ("Unique source files", f"{df['source_file'].nunique():,}", "Per-exposure catalog files"),
            ("UTC nights", f"{df['night'].nunique():,}", "Parsed from source_file"),
            ("Inferred fields", f"{df['field_id'].nunique():,}", "Parsed from source_file prefix"),
            ...
            ("Median Mag_Kron", fmt(q(df["Mag_Kron"], 50), 4), "Measured catalog magnitude"),
            ("Median MagErr_Kron", fmt(q(df["MagErr_Kron"], 50), 4) + " mag", "Catalog uncertainty"),
            ("Median Kron S/N proxy", fmt(q(df["snr_kron_proxy"], 50), 4), "Flux_Kron / FluxErr_Kron"),
            ...
            ("Horizons failed rows", f"{int(truthy(df['horizons_failed']).sum()):,}", "JPL query flag"),
        ],
    ...
    orbit = (
        ...
        .agg(
            ...
            **{
                ...
                "Median Mag_Kron": ("Mag_Kron", "median"),
    ...
    orbit.columns = ["Code", "Orbit class", "Detections", "Unique objects", "Median Mag_Kron", "Median separation"]
    orbit["Detections"] = orbit["Detections"].map(lambda x: f"{x:,}")
    orbit["Unique objects"] = orbit["Unique objects"].map(lambda x: f"{x:,}")
    orbit["Median Mag_Kron"] = orbit["Median Mag_Kron"].map(lambda x: fmt(x, 3))
    ...
    work["mag_bin"] = pd.cut(work["Mag_Kron"], mag_bins)
    mag_ast = (
        ...
        .agg(
            ...
            **{
                ...
                "Median mag. error": ("MagErr_Kron", "median"),
            },
        )
        .reset_index()
    )
    ...
    rate_ast = rate_ast[["Rate bin", "Detections", "Median separation", "RMS separation", "Median Mag_Kron"]]
    for col in ["Median separation", "RMS separation", "Median Mag_Kron"]:
    ...
    nightly.columns = ["UTC date", "Detections", "Unique objects", "Source files", "Median Mag_Kron", "Median separation"]
    for col in ["Detections", "Unique objects", "Source files"]:
        nightly[col] = nightly[col].map(lambda x: f"{x:,}")
    for col in ["Median Mag_Kron", "Median separation"]:
    ...
    obj = obj[["query_id", "Name", "Detections", "Distinct nights", "First epoch", "Last epoch", "Baseline days", "Median Mag_Kron", "Median separation", "Orbit class"]]
    obj.columns = ["Object ID", "Name", "Detections", "Distinct nights", "First MJD", "Last MJD", "Baseline days", "Median Mag_Kron", "Median separation", "Orbit class"]
    for col in ["First MJD", "Last MJD", "Baseline days", "Median Mag_Kron", "Median separation"]:
        obj[col] = obj[col].map(lambda x: fmt(x, 3))

    write_csv_and_latex(overall, outdir / "overall_statistics.csv", "Overall statistics of the GOTTA known-asteroid matched table.", "tab:summary", "lll")
    ...
    write_csv_and_latex(mag_ast, outdir / "astrometry_by_magnitude.csv", "Astrometric residuals as a function of measured Kron magnitude. Separations are in arcsec.", "tab:mag_astrometry", "lrrrr")
    ...
    write_csv_and_latex(nightly, outdir / "nightly_top10.csv", "Ten UTC nights with the largest numbers of matched known-asteroid detections.", "tab:nightly_top10", "lrrrrr")
    write_csv_and_latex(obj, outdir / "most_observed_objects.csv", "Most frequently observed known asteroids in the matched table.", "tab:most_observed", "llrrrrrrrl")
...
def plot_photometry(df: pd.DataFrame, outdir: Path) -> None:
    ...
    histogram(axes[0, 0], finite(df["Mag_Kron"]), np.linspace(10, 22, 49), "Mag_Kron [mag]")
    histogram(axes[0, 1], finite(df["MagErr_Kron"]), np.linspace(0, 0.8, 49), "MagErr_Kron [mag]", logy=True, color="#59a14f")
    histogram(axes[1, 0], finite(df["snr_kron_proxy"]), np.logspace(0, 3, 50), "Kron S/N proxy", logy=True, color="#f28e2b")
    axes[1, 0].set_xscale("log")
    hexbin(axes[1, 1], df["Mag_Kron"], df["MagErr_Kron"], "Mag_Kron [mag]", "MagErr_Kron [mag]", gridsize=50)
    fig.tight_layout()
...
def plot_astrometry(df: pd.DataFrame, outdir: Path) -> None:
    ...
    hexbin(axes[1, 1], df["Mag_Kron"], df["sep_arcsec"], "Mag_Kron [mag]", "Separation [arcsec]", gridsize=55)
    axes[1, 1].set_ylim(0, 1.5)
    fig.tight_layout()
    save_figure(fig, outdir / "astrometric_residuals")

    fig, ax = plt.subplots(figsize=(10, 8))
    hexbin(ax, df["ang_rate_arcsec_hour"], df["sep_arcsec"], r"Angular rate [arcsec hr$^{-1}$]", "Separation [arcsec]", gridsize=55, xscale="log")
    ax.set_ylim(0, 1.5)
    fig.tight_layout()
...
def plot_geometry(df: pd.DataFrame, outdir: Path) -> None:
    ...
    histogram(axes[0, 1], finite(df["phase_deg"]), np.linspace(0, 90, 55), "Phase angle [deg]", color="#59a14f")
...
def plot_temporal(df: pd.DataFrame, outdir: Path) -> None:
    ...
    axes[0, 0].bar(x, nightly.to_numpy(), color="#4c78a8")
    step = max(1, len(x) // 9)
    axes[0, 0].set_xticks(x[::step])
    axes[0, 0].set_xticklabels(nightly.index[::step], rotation=45, ha="right")
...
def plot_example(df: pd.DataFrame, outdir: Path) -> None:
    ...
    axes[1, 1].scatter(sub["mag"], sub["Mag_Kron"], s=55, alpha=0.8, color="#4c78a8", edgecolor="black", linewidth=0.3)
    axes[1, 1].set_xlabel("Predicted magnitude [mag]")
    axes[1, 1].set_ylabel("Mag_Kron [mag]")
    axes[1, 1].tick_params(labelsize=22)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate v2 paper figures and tables from gotta_asteroids.fits.")
    parser.add_argument("fits_path", nargs="?", default="gotta_asteroids.fits")
    parser.add_argument("--outdir", default="paper_draft")
    args = parser.parse_args()

    root = Path(args.outdir)
    figdir = root / "figures_v2"
    tabledir = root / "tables_v2"
    setup_style()
