...
def show_image_pos(image, x1=None, x2=None, y1=None, y2=None,
               s1=30,s2=50,c1='r',c2='b',
               percl=99, percu=None, is_mask=False,
               figsize=(10, 10),
               cmap='gray', log=False, clip=True,
               show_colorbar=True, show_ticks=True,
    ...
    if vmin is not None or vmax is not None:
        scale_args = dict(vmin=vmin, vmax=vmax)

    im = ax.imshow(reduced_data, origin='lower',
                   cmap=cmap, extent=extent, aspect='equal', **scale_args)

    if x1 is not None and y1 is not None:
        ax.scatter(x1,y1,marker='o',edgecolors=c1,s=s1,color='none')

    if x2 is not None and y2 is not None:
        ax.scatter(x2,y2,marker='o',edgecolors=c2,s=s2,color='none')
