...
\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{figures_v5/sitian_pilot_asteroid_statistics.pdf}
...
\begin{figure}
\centering
\includegraphics[width=0.58\textwidth]{figures_v5/method_flowchart_styled.pdf}
...
\input{tables_v5/overall_statistics.tex}
...
\input{tables_v5/orbit_class_statistics.tex}

The sky distribution in Figure~\ref{fig:sky_distribution} reflects the combination of the prototype survey footprint, seasonal visibility, and the instantaneous distribution of known minor planets during the observing campaign. The orbital distribution in Figure~\ref{fig:orbit_distribution} confirms that the sample is dominated by main-belt objects, with smaller contributions from inner and outer main-belt asteroids, Jupiter Trojans, Mars-crossers, and NEOs.

\begin{figure}
\centering
\includegraphics[width=0.95\textwidth]{figures_v5/gotta_radec_healpix_nside64.pdf}
...
\begin{figure*}
\centering
\includegraphics[width=0.85\textwidth]{figures_v5/asteroid_orbits.pdf}
...
\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{figures_v5/astrometric_residuals.pdf}
...
\input{tables_v5/astrometry_by_magnitude.tex}
...
\begin{figure}
\centering
\includegraphics[width=0.95\columnwidth]{figures_v5/separation_vs_rate.pdf}
...
\input{tables_v5/astrometry_by_rate.tex}
...
The scientific value of a high-cadence survey for asteroid work depends not only on the number of recovered objects, but also on how often individual objects are revisited. In the current sample the revisit statistics are highly non-uniform. The median number of detections per object is two, the mean is 3.50, and the 84th percentile is six. A small subset of objects has much denser sampling; the most frequently recovered object has 317 detections over nine distinct nights. Table~\ref{tab:most_observed_objects} lists the five most frequently recovered objects.

\input{tables_v5/most_observed_objects.tex}

These repeated detections are useful for testing the construction of asteroid time series and for checking internal consistency of the photometry. However, they do not yet imply that the prototype-only survey can deliver reliable rotation periods for most recovered asteroids. The cumulative distributions in Figure~\ref{fig:temporal_sampling} show that most objects have sparse sampling and limited phase coverage. This motivates the auxiliary 60 cm Schmidt observations described in Section~\ref{sec:lightcurve}, which are used to validate the period-search part of the pipeline on selected targets.

\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{figures_v5/temporal_sampling.pdf}
...
\input{tables_v5/nightly_top5.tex}
...
\begin{figure*}
\centering
\includegraphics[width=0.95\textwidth]{figures_v5/example_crossmatch_diagnostic.pdf}
...
The repeated detections analyzed above provide the first estimate of the time-domain sampling available for known asteroids in the GOTTA Prototype survey. However, the prototype-only sample remains too sparse for robust rotation-period determination for most recovered objects. The median number of detections per object is only two, and even though a small number of objects have many repeated measurements, these cases are not representative of the full sample. Consequently, the present GOTTA-only catalog is most useful for validating the construction of sparse asteroid time series rather than for deriving a large set of reliable periods.

To test the period-search component of the asteroid pipeline, we therefore combine selected GOTTA Prototype measurements with additional observations obtained by a nearby 60 cm Schmidt telescope. The purpose of this combined analysis is to validate the light-curve workflow under realistic observing conditions. The period-search procedure will compare several independent methods, such as Lomb--Scargle analysis, phase-dispersion minimization, and string-length or related phase-folding statistics. Only objects for which multiple methods produce consistent periods and visually acceptable phased light curves will be retained as reliable period measurements.
...
Software: Astropy \citep{Astropy2013,Astropy2018,Astropy2022}, NumPy \citep{Harris2020}, Matplotlib \citep{Hunter2007}, pandas \citep{McKinney2010}, astroquery \citep{Ginsburg2019}, HEALPix \citep{Gorski2005}, SExtractor \citep{Bertin1996}, SCAMP \citep{Bertin2006}, astrometry.net \citep{Lang2010}.
