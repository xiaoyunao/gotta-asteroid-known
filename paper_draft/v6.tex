...
\begin{figure}[!htbp]
\centering
\includegraphics[height=0.76\textheight,keepaspectratio]{figures_v6/known_object_processing.png}
...
The combined GOTTA and 60 cm Schmidt data are used to identify asteroid light curves with plausible double-peaked rotational modulation. The current analysis finds 26 candidate objects with double-peaked morphology. Among them, 11 objects cover more than one rotation cycle and are regarded as high-confidence period measurements under the adopted pipeline criteria, while the remaining 15 candidates show double-peaked structure but require additional observations for confirmation. None of the 26 candidate periods has a corresponding reference period in the external period database used in the current comparison. This statement should be checked against the final database query before submission and should not be interpreted as a complete claim of newly discovered periods.

Figure~\ref{fig:lightcurve_examples} shows the folded light curves for the 11 high-confidence objects. The panel titles list the initial period and the adopted single- or double-peaked period used for the displayed phase folding. The black curves show the Fourier models used for the morphology check. Table~\ref{tab:period_results} summarizes the high-confidence period measurements selected by the consistency and visual-inspection criteria. The final table should include the object identifier, object name, numbers of GOTTA and 60 cm Schmidt photometric points, observing time span, adopted rotation period, approximate period uncertainty, light-curve amplitude, adopted period model, photometric measurement, comparison with any literature period, and a quality flag. These results demonstrate that, although the GOTTA Prototype data alone are generally too sparse for period determination, the recovered known-object catalog can be combined with auxiliary observations to validate the asteroid period-analysis pipeline.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{figures_v6/lightcurve.png}
\caption{Phase-folded light curves for the 11 high-confidence period measurements in the combined GOTTA Prototype and 60 cm Schmidt validation sample. Blue and cyan points denote the photometric measurements used in the folded light curves, gray points mark rejected or lower-weighted measurements, and black curves show the adopted Fourier models. Each panel gives the object identifier and the candidate period information used in the current period analysis.}
...
Software: Astropy \citep{Astropy2013,Astropy2018,Astropy2022}, NumPy \citep{Harris2020}, SciPy \citep{Virtanen2020}, Matplotlib \citep{Hunter2007}, pandas \citep{McKinney2010}, astroquery \citep{Ginsburg2019}, healpy/HEALPix \citep{Gorski2005}, and PyAstronomy.
