...
The combined GOTTA and 60 cm Schmidt data are used to identify asteroid light curves with plausible double-peaked rotational modulation. The current analysis finds 26 candidate objects with double-peaked morphology. After the final sampling and visual-inspection cuts, 21 objects are retained in the period-analysis validation table. Fourteen of these are assigned reliable final quality flags, while seven are retained as questionable solutions that require additional observations for confirmation. None of the 26 candidate periods has a corresponding reference period in the external period database used in the current comparison. This statement should be checked against the final database query before submission and should not be interpreted as a complete claim of newly discovered periods.

Figure~\ref{fig:lightcurve_examples} shows folded light curves for selected high-quality objects. The panel titles list the initial period and the adopted single- or double-peaked period used for the displayed phase folding. The black curves show the Fourier models used for the morphology check. Table~\ref{tab:period_results} summarizes the final period-analysis validation table, including the source-file counts for GOTTA and 60 cm Schmidt detections, the effective number of retained photometric points, the adopted period, period uncertainty, period model, photometric measurement type, and final quality flag. These results demonstrate that, although the GOTTA Prototype data alone are generally too sparse for period determination, the recovered known-object catalog can be combined with auxiliary observations to validate the asteroid period-analysis pipeline.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{figures_v6/lightcurve.png}
\caption{Phase-folded light curves for selected high-quality period measurements in the combined GOTTA Prototype and 60 cm Schmidt validation sample. Blue and cyan points denote the photometric measurements used in the folded light curves, gray points mark rejected or lower-weighted measurements, and black curves show the adopted Fourier models. Each panel gives the object identifier and the candidate period information used in the current period analysis.}
