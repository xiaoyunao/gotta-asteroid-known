...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures_v7/sitian_pilot_asteroid_statistics.pdf}
...
The filtered predictions are then associated with the GOTTA source catalog. For each predicted position, the nearest catalog source is identified within the adopted arcsecond-level matching radius. This radius is chosen to be larger than the expected centroiding uncertainty of faint moving sources but small enough to suppress random associations in crowded fields. The observed-minus-predicted residuals reported below are computed from the small-angle offsets in right ascension and declination after the accepted matches have passed the Gaia stationary-source filter. The angular separation is computed on the sphere, with the right-ascension difference projected by \(\cos\delta\) for small-angle residuals.

A Gaia-based stationary-source mask is applied after the asteroid-source association. A predicted asteroid position may occasionally fall close to a field star, in which case the nearest-neighbor source in the GOTTA catalog can be the stationary star rather than the moving object. To suppress this contamination, every accepted asteroid-source association is cross-matched with Gaia DR3 \citep{Gaia2023}. If the same GOTTA source is also matched to a Gaia source within 1.5 arcsec, the association is rejected from the asteroid sample. This simple mask removes obvious stellar contaminants before the astrometric residuals are analyzed as a function of brightness and sky-plane angular rate. This step is applied before all residual and rate-dependent statistics reported below.
...
\input{tables_v7/overall_statistics.tex}

The orbit-class composition is shown in Table~\ref{tab:orbit_class}. Main-belt asteroids account for 51,058 detections and 14,674 unique objects. Outer main-belt asteroids, Jupiter Trojans, inner main-belt asteroids, and Mars-crossing asteroids make up most of the remaining sample. Classes with only a few detections are grouped as Other/unclassified, including the generic SBDB class AST, which denotes an asteroid not assigned to a more specific dynamical class in the adopted mapping. The NEO classes are represented by Amor, Apollo, and Aten objects. The dominance of main-belt objects is expected because the survey footprint and limiting depth preferentially sample objects near ordinary main-belt opposition geometries. The smaller NEO and PHA subsets should therefore be viewed as validation samples for the recovery path rather than as indicators of discovery performance.

\input{tables_v7/orbit_class_statistics.tex}
...
\input{tables_v7/nightly_top5.tex}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures_v7/gotta_radec_healpix_nside64.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures_v7/asteroid_orbits.pdf}
...
The corresponding observing geometry is summarized in Figure~\ref{fig:geometry_distribution}. The median heliocentric distance is 2.581 AU, the median topocentric distance is 1.840 AU, and the median phase angle is 13.245 deg. These values are consistent with a sample dominated by ordinary main-belt observing geometries and explain why the main angular-rate distribution is relatively narrow. Although the sample is dominated by main-belt asteroids, the recovered NEO/PHA subset provides a useful stress test because these objects tend to have higher apparent rates and more variable observing geometry. The NEO subset contains 259 detections of 77 unique objects, with median \(g_{\rm aper}=17.525\) mag, median angular rate \(111.643\) arcsec hr\(^{-1}\), and median residual separation 0.232 arcsec. The PHA subset contains 53 detections of 19 unique objects, with median \(g_{\rm aper}=17.660\) mag, median angular rate \(132.424\) arcsec hr\(^{-1}\), and median residual separation 0.211 arcsec. These small subsets are not large enough for a formal population-level performance assessment, but they show that the current association path already covers higher-rate Solar System targets.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures_v7/motion_geometry.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures_v7/photometric_statistics.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures_v7/separation_vs_rate.pdf}
...
The magnitude and rate dependences are shown in Figures~\ref{fig:astro} and \ref{fig:sep_rate}, where the running medians summarize the main trends without introducing additional binned tables.

These residuals should be interpreted as association-level astrometric diagnostics rather than as a direct measurement of orbit improvement. For most well-observed main-belt asteroids, the predicted ephemeris uncertainty is expected to be smaller than the centroiding uncertainty of the faint GOTTA detections, so the observed-minus-predicted residual mainly traces the positional precision of the recovered survey source. However, no orbit refitting is performed in this work. The results therefore demonstrate that the prototype catalogs can provide sub-arcsecond asteroid astrometry suitable for known-object association and potential future orbit-refinement experiments, but they do not by themselves quantify the improvement of minor-planet orbits.
...
\input{tables_v7/most_observed_objects.tex}

These repeated detections are useful for constructing sparse asteroid time series and for checking the internal consistency of the photometry. However, the prototype-only sampling is not sufficient for robust period determination for most objects, as shown by the cumulative detection-count distribution in Figure~\ref{fig:temporal_sampling}. This motivates the auxiliary light-curve experiment in Section~\ref{sec:lightcurve}, where the GOTTA detections provide part of the input time series and the nearby 60 cm Schmidt telescope supplies denser temporal coverage for selected targets.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.90\textwidth]{figures_v7/temporal_sampling.pdf}
\caption{Cumulative distribution of the number of recovered detections per object. Most objects are recovered only a few times, while a small tail has tens to hundreds of detections and is useful for light-curve pipeline tests.}
...
Asteroid light curves provide rotation periods, amplitudes, and basic constraints on shape and spin state, and they are therefore an essential downstream product of a moving-object survey pipeline. The known-object recovery described above supplies object identities, observing epochs, and calibrated photometry, but the prototype survey cadence alone does not provide sufficient phase coverage for most targets. We therefore use auxiliary observations from a nearby 60 cm Schmidt telescope to test the period-search workflow on selected asteroids. The combined data set is used as a validation sample rather than as a complete rotational survey. GOTTA Prototype measurements provide part of the time series and the object-association framework, while the 60 cm Schmidt observations provide denser sampling for the selected targets.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{figures_v7/lightcurve_reliable.png}
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth,height=0.86\textheight,keepaspectratio]{figures_v7/lightcurve_questionable.png}
...
\input{tables_v7/period_reliable_objects.tex}
